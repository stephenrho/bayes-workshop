...
- Full example in examples/brms-example1.R
...
```{r}
ppc_stat(y=sleepstudy$Reaction, yrep = yrep)
```
...
## `marginal_effects` (back to brms)

```{r}
x = marginal_effects(m1)
plot(x)
```
...
```{r}
stanplot(m2, pars = 'r_Subject') # use parnames(m2) for all pars
```
...
```{r}
m2_days = posterior_samples(m2, pars = "b_Days")[,1]
```

```{r, echo=F}
hist(m2_days, breaks = 30, border = F, col="lightblue", main="Change in Reaction per Day", xlab="", ylab="", axes=F)
axis(1)
lines(x = quantile(m2_days, probs = c(.025, .975)), y=c(0,0), lwd=3)
text(mean(m2_days), 60, labels = sprintf("Mean + 95%% CI:\n%.2f [%.2f, %.2f]", mean(m2_days), 
                              quantile(m2_days, probs = c(.025)), 
                              quantile(m2_days, probs = c(.975))))
```
...
- This script builds on what we have been looking at so far
- Let's run through
...
- Instead `mo()` allows you to model the effect of the predictor to be monotonically increasing or decreasing (but not linear)
